...
class BudgetExceededError(Exception):
    ...
    def __init__(self, limit_usd: float, actual_usd: float):
        ...
        super().__init__(
            f"[AgentGate] Budget cap of {format_cost(limit_usd)} exceeded "
...
class BudgetTracker:
    ...
    def record(
        ...
        if (
            ...
            log.warning(
                f"[AgentGate] Budget alert: session spend {format_cost(self.session_cost)} "
...
def _check_budget(options: AgentGateOptions) -> Optional[dict]:
    ...
    if options.budget_alert_usd is not None and current >= options.budget_alert_usd:
        log.warning(
            f"[AgentGate] 💰 Budget alert: ${current:.4f} of ${options.max_cost_usd:.2f} cap used "
...
def _check_loops(tool_name: str, args: Any, options: AgentGateOptions) -> Optional[dict]:
    """
...
def _record_cost(options: AgentGateOptions, response: dict) -> None:
    ...
    if cost and options.max_cost_usd is not None:
        ...
        log.debug(
            f"[AgentGate] Cost recorded: +${cost:.6f} "
            f"(session total: ${_session_costs[session_id]:.4f})"
        )


def _poll_approval(request_id: str, options: AgentGateOptions) -> dict:
    ...
    log.warning(f"[AgentGate] ⏳ Action requires human intervention. Waiting for approval...")
    log.warning(f"[AgentGate] 👉 Approve at: {options.dashboard_api_url}/approvals")
        ...
        try:
            with httpx.Client(timeout=5.0) as client:
                ...
                if resp.status_code == 200:
                    ...
                    if status == "approved":
                        log.info("[AgentGate] ✅ Action APPROVED by human. Resuming...")
                        return {"decision": "ALLOW"}
                    if status == "denied":
                        log.error("[AgentGate] ❌ Action DENIED by human.")
                        return {"decision": "DENY", "reason": "Explicitly denied by human administrator."}
        except Exception as e:
            log.debug(f"[AgentGate] Polling error: {e}")
...
async def _poll_approval_async(request_id: str, options: AgentGateOptions) -> dict:
...
def _handle_decision(decision: str, reason: Optional[str], tool_name: str) -> Optional[str]:
    ...
    if decision == "REQUIRE_APPROVAL":
        ...
        return (
            ...
            "Check your AgentGate dashboard at https://agentgate.ai/"
        )
...
def protect(agent_or_runnable: Any, options: AgentGateOptions) -> Any:
    """
    ...
    Args:
        ...
        options: AgentGateOptions for configuration.
    """
...
def wrap_crewai(agent_or_crew: Any, options: AgentGateOptions) -> Any:
...
def wrap_autogen(agent: Any, options: AgentGateOptions) -> Any:
    ...
    def check(self, tool_name: str, args: Any, next_fn: callable) -> Any:
        ...
        try:
            ...
            if blocked is not None:
                return blocked
            result = next_fn()
            log.debug(f"[AgentGate] {self.budget.summary}")
            return result
        except httpx.RequestError as e:
            log.error(f"[AgentGate] Network error: {e}")

...
def secure(api_key: Optional[str] = None, **options):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            sw_options = AgentGateOptions(api_key=api_key, **options)
            return universal_protect(agent, sw_options)
        return wrapper
    return decorator

def wrap_langchain(agent: Any, options: AgentGateOptions) -> Any:
    """Native LangChain callback injection."""
    callback = AgentGateLangChainCallback(options)

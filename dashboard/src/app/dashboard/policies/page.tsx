...
import { ShieldAlert, PlusCircle, Sparkles, Loader2 } from "lucide-react";
                    ...
                    <CardHeader className="bg-white/[0.01] border-b border-white/[0.05]">
                        <div className="flex justify-between items-center w-full">
                            ...
                            <Button
                                ...
                                className={`text-xs border-indigo-500/30 hover:bg-indigo-500/10 transition-colors ${showAiWizard ? 'bg-indigo-500/10 text-indigo-300' : 'bg-transparent text-indigo-400'}`}
                            >
                    ...
                    <CardContent className="pt-6">
                            ...
                            {showAiWizard && (
                                ...
                                >
                                    <div className="p-4 rounded-xl border border-indigo-500/20 bg-indigo-500/5 space-y-3">
                                        <label className="text-sm font-medium text-indigo-300">Describe restriction in plain English</label>
                                        <Textarea
                                            value={aiPrompt}
                                            onChange={e => setAiPrompt(e.target.value)}
                                            placeholder="e.g., Stop the agent from deleting files."
                                            className="bg-black/50 border-indigo-500/30 text-white placeholder:text-indigo-200/50 resize-none focus-visible:ring-indigo-500/50"
                                            rows={2}
                                        />
                                        <Button
                                            type="button"
                        ...
                        <form onSubmit={handleCreatePolicy} className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                />
                                <p className="text-xs text-indigo-400/80 italic mt-1">Generated by AI. Please review before saving.</p>
                            </div>
                            <div className="space-y-2">
                                ...
                                <Select value={ruleType} onValueChange={(v: RuleType) => setRuleType(v)}>
                                    <SelectTrigger className="w-full bg-neutral-900 border-white/10 text-white focus:ring-1 focus:ring-indigo-500 hover:border-white/20 transition-colors">
                            ...
                            <motion.div whileHover={(!selectedAgentId || !toolName || !condition) ? {} : { scale: 1.02 }} whileTap={(!selectedAgentId || !toolName || !condition) ? {} : { scale: 0.98 }}>
                                <Button type="submit" disabled={!selectedAgentId || !toolName || !condition} className="w-full bg-indigo-600 hover:bg-indigo-500 text-white shadow-lg shadow-indigo-500/20 transition-all font-medium mt-4 disabled:opacity-50 disabled:shadow-none">
                                    ...
                                    <TableBody>
                                        {policies.map((p) => (
                                            <TableRow key={p.id} className="border-white/[0.02] hover:bg-white/[0.02] transition-colors group/row">
                                                ...
                                                <TableCell className="font-mono text-xs max-w-[200px] truncate text-indigo-400 py-4 group-hover/row:text-indigo-300 transition-colors">
                                                    <span className="bg-indigo-500/10 px-2 py-1 rounded-md border border-indigo-500/20">{p.condition}</span>
                                                </TableCell>

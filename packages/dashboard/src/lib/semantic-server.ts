...
// ---------------------------------------------------------------------------
// Layer 2: AI Semantic Analysis — Dashboard (Next.js) adapter
// ---------------------------------------------------------------------------
// Mirrors the server's semantic.ts but uses the dashboard's DB pool.
// Shared logic is intentionally duplicated (not cross-package imported)
// to keep the Next.js bundle independent of the Express server package.
// ---------------------------------------------------------------------------
...
export async function analyzeCall(
    ...
    ).catch(err => console.error('[Semantic] Failed to log analysis', err));
...
// ── Behavioral Anomaly (inline for dashboard) ──

async function computeAnomalyScore(req: SemanticAnalysisRequest): Promise<number> {
    try {
...
export async function updateBaseline(
    tenantId: string, agentId: string, toolName: string, args: unknown
): Promise<void> {
...
async function callStandardLLM(prompt: string): Promise<{ score: number; reasoning: string; model: string }> {
    ...
    if (!apiKey) return { score: 0, reasoning: 'Layer 2 not configured', model: 'none' };
...
async function callCustomEndpoint(tenantId: string, endpoint: string, prompt: string): Promise<{ score: number; reasoning: string; model: string }> {

...
[![PyPI](https://img.shields.io/pypi/v/langchain-suprawall?label=pypi&color=B8FF00)](https://pypi.org/project/langchain-suprawall/)
[![npm](https://img.shields.io/npm/v/suprawall-langchain?label=npm&color=B8FF00)](https://www.npmjs.com/package/suprawall-langchain)
...
<!-- LAUNCH VIDEO — GitHub READMEs don't render <iframe>, so we use a linked YouTube thumbnail. -->
<!-- Local GIF fallback renders inline on email/RSS/social cards. -->
<a href="https://youtu.be/BOkKkhe9rmY" target="_blank" rel="noopener">
  <img src="https://img.youtube.com/vi/BOkKkhe9rmY/hqdefault.jpg" alt="Watch: SupraWall — The Open-Source Security Perimeter for AI Agents (40s)" width="760">
</a>
...
AI agents now write code, spend money, query databases, and take real-world actions on your behalf — autonomously. The frameworks that orchestrate them ( OpenClaw, LangChain, CrewAI, AutoGen, Claude Code) are excellent at *making them productive*. None of them are responsible for *making them safe*.
...
And with the EU AI Act enforcement deadline on **August 2, 2026**, we ship [8 pre-built sector templates](#eu-ai-act-compliance--shipped-not-promised) covering every Annex III high-risk category — HR, healthcare, education, critical infrastructure, biometrics, law enforcement, migration, justice — plus a DORA template for financial services. Compliance by `pip install`, not by 200-page PDF. (Framework-agnostic Python SDK coming Q2 2026).
...
> [!NOTE]
> We are currently launching with the **LangChain** and **smolagents** Python adapters. A unified, framework-agnostic Python SDK is scheduled for Q2 2026.
...
```typescript
import { withSupraWall } from "suprawall-langchain";
import { agent } from "./my-agent";

const safe = withSupraWall(agent, { policy: "./policies/langchain-safe.json" });
...
|---|---|---|
| LangChain (Python) | ✅ First-class | `pip install langchain-suprawall` |
| LangChain / LangGraph (TS) | ✅ First-class | `npm install suprawall-langchain` |
| Vercel AI SDK | ✅ First-class | `npm install suprawall-vercel-ai` |
| OpenClaw (browser agents) | ✅ First-class | `npm install suprawall-claw` |
| MCP / Claude Code | ✅ First-class | `npm install suprawall-mcp-plugin` |
| CrewAI / AutoGen | ✅ Via core SDK | `pip install langchain-suprawall` — callback API is framework-agnostic |
| Custom / homegrown | ✅ | One-line `withSupraWall()` / `secure_agent()` wrapper |

Languages: Python, TypeScript, Go. More via the MCP plugin.
...
[Website](https://www.supra-wall.com) · [Docs](https://www.supra-wall.com/docs) · [Cloud](https://www.supra-wall.com/cloud) · [Blog](https://www.supra-wall.com/blog) · [X / @AlexPeghin](https://x.com/AlexPeghin) · [License: Apache 2.0](LICENSE)

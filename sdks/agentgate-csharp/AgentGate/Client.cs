...
namespace AgentGate.Models
{
    ...
    public class Client
    {
        ...
        {
            apiKey = key ?? Environment.GetEnvironmentVariable("AGENTGATE_API_KEY");
            apiUrl = url ?? Environment.GetEnvironmentVariable("agentgate_API_URL") ?? "https://api.agentgate.io/v1/evaluate";

            if (string.IsNullOrEmpty(apiKey)) throw new ArgumentException("AGENTGATE_API_KEY is required");
        }
        ...
        public async Task<EvaluateResponse> EvaluateAsync(string agentId, string toolName, object arguments = null)
        {
            ...
            {
                throw new HttpRequestException("AgentGate Network Error: Failing closed.");
            }
            ...
            {
                throw new PolicyViolationException($"AgentGate Policy Violation: Tool '{toolName}' explicitly denied.");
            }
            else if (result.Decision == "REQUIRE_APPROVAL")
            {
                throw new PolicyViolationException($"AgentGate Policy Violation: Tool '{toolName}' requires human approval.");
            }
